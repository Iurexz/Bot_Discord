...
import { ApplicationCommandOptionType, ApplicationCommandType, codeBlock, flatten, roleMention } from "discord.js";

const moderationRoleId = "1169092105625419807";
...
export default new Command({
    ...
    options: [
        ...
        {
            name:"mensagem",
            description:"Deletar mensagem especifica do canal",
            type: ApplicationCommandOptionType.String,
            autocomplete:true,
        }
    ],
...
async autoComplete(interaction) {
    ...
    async run(interaction){
        ...
        if(!member.roles.cache.has(moderationRoleId)){
            interaction.editReply({ content: `Apenas membros com o cargo ${roleMention(moderationRoleId)} podem utilizar este comando!`});
            return;
        }
        if(!channel?.isTextBased()){
            interaction.editReply({ content:"Não é possivel utilizar este comando no canal!"});
            return;
        }
        ...
        if (mention){
            ...
            .then (cleared => interaction.editReply({
                content: cleared.size?`${cleared.size} mensagens de ${mention} deletadas com sucesso!`
        ...
        channel.bulkDelete(Math.min(amount, 100))
        .then (cleared => interaction.editReply({
            content: cleared.size?`${cleared.size} mensagens deletadas com sucesso!`
